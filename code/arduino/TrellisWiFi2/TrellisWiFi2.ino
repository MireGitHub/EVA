...
IPAddress server(192,168,240,155); // 
YunClient client;
String parametri ="";
...
void setup() {
  ...
  for (uint8_t i=0; i<numKeys; i++) {
   ...
   { 
       Serial.println("connected setup");
    } else {
      Serial.println("connection failed");
    }
}

void send_val(int state,int i){
 //if (client.connect(server, 8080)) 
  //{
    
    // '{"state":state ,"index":i}'
    parametri="{state:"+String(state)+",index:"+String(i)+"}";
    
    client.println("POST /post1 HTTP/1.1");
    client.print("Content-length:");
    client.println(parametri.length());
    Serial.println(parametri.length());
    Serial.println(parametri);
    client.println("Connection: Close");
    client.println("Host:92.168.240.155");
